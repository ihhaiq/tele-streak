...
يُمرّر رقم حساب البوت إلى خدمة القنوات حتى تُهمل رسائله الذاتية. تبقى هوية
الشخص الذي نشر باسم القناة محدودة بما يرسله Telegram في `author_signature`؛
الـBot API لا يضمن كشف الحساب الحقيقي لكل منشور.

نشر ستوري القناة نفسه غير منفّذ حاليًا: `postStory` في
[Telegram Bot API](https://core.telegram.org/bots/api#poststory) مخصص لحساب
Business مُدار، وليس للقناة. دالة `can_manage_channel_story` هي فحص صلاحيات
مستقل ومختبر، وليست مسار نشر: تقبل المالك أو مشرفًا يملك `can_post_stories`،
وترفض العضو العادي وفشل قراءة العضوية. يجب استدعاؤها مجددًا عند المعاينة
والنشر إذا أضيف ناقل يدعم نشر القنوات لاحقًا. مسار ستوري الخاص يبقى كما هو.

## أسماء أطراف الستريك وحالة اليوم

الحالة في الخاص تعرض `✅ أكمل اليوم: حسين` و`⏳ بانتظار: أحمد`، وبعد إرسال
الاثنين تعرض `✅ اكتمل اليوم بواسطة: حسين وأحمد`. يتحدد الإكمال من إرسال
اليوم حسب وضع الستريك وتوقيت الاتصال؛ الحماية أو الإحياء لا يُعرضان كإرسال
جديد من الشخصين. المهام الفردية تعرض اسم صاحبها، والمشتركة تعرض الاسمين،
وكذلك المقارنة والتنبيهات وطلب الإحياء. إذا ما متوفر اسم، يظهر «الطرف الأول»
أو «الطرف الثاني» بدون اختلاق اسم حساب.

- `participant_accounts` يحفظ `user_id` والاسم عند استلام بيانات الحساب.
  الاسم الفارغ لا يمسح الاسم المعروف، وتبقى أسماء ملفات المغامرة القديمة
  المحفوظة في قاعدة البيانات متاحة كبديل.
- `last_contributor_user_id` و`last_contributor_name` و`last_contribution_day`
  تحفظ آخر شخص حقق شرط إرسال اليوم ضمن معاملة تسجيل الرسالة نفسها.
  إرسال إضافي من شخص أكمل شرطه سابقًا لا يبدل المنجز. المنتظر محسوب من
  `owner_sent_day` و`peer_sent_day` وهوية الحسابين المحفوظة، بدون cache للحالة.
- القنوات تحفظ توقيع المنجز، ومعه `last_completed_by_user_id` إذا كشفه
  Telegram. لا يمكن استنتاج حساب الناشر أو كون المنشور آليًا إذا وصل باسم
  القناة فقط؛ الاستبعاد يقتصر على هوية البوت التي يكشفها التحديث.
- الترقية تلقائية عند التشغيل، تضيف جدولًا وأعمدة nullable وقابلة للتكرار؛
  لا تمسح الستريكات أو XP أو المهام. لا تُختلق هوية المنجز للأيام القديمة.
- المشروع يدعم **SQLite فقط** عبر `DATABASE_PATH`، ولا يقرأ `DATABASE_URL`
  ولا يملك مسار Postgres. يجب أن يكون ملف SQLite على volume دائم في الاستضافة.
- الاختبارات: `python -m pip install -r requirements-dev.txt` ثم
  `python -m pytest -q`. تتضمن ترقية schema قديم وإعادة التشغيل وإعادة الربط،
  ومسارات الضيف ورفض rich والخاص والقنوات وصلاحيات الستوري.
