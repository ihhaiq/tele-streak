...
class PoseCatalog:
    ...
    def __init__(self, assets_dir: Path, *, rng: random.Random | None = None):
        ...
        self._by_id = {pose.id: pose for pose in self._poses}
        self._normal = tuple(p for p in self._poses if p.category == "normal")
        self._milestone = tuple(p for p in self._poses if p.category == "milestone")
    ...
    def get(self, pose_id: str) -> Pose:
        try:
            return self._by_id[pose_id]
        except KeyError:
            raise KeyError(f"Unknown pose: {pose_id}") from None

    def _eligible(self, days: int) -> Iterable[Pose]:
        exact = [pose for pose in self._poses if days in pose.milestones]
        if exact:
            return exact
        if days in MILESTONES and self._milestone:
            return self._milestone
        return self._normal or self._poses
