...
class Repository:
    ...
    async def upsert_connection(
        ...
        Telegram may issue a new business_connection_id when the same owner
        disconnects and reconnects the bot. Streak state belongs to the owner
        and chat, not to that transient connection id, so an empty new
        connection inherits the most recently used connection for that owner.
        """
        now = self._now()
        async with self.database.connect() as db:
            ...
            connection_exists = await cursor.fetchone() is not None

            old_connection_id: str | None = None
            if is_enabled and not connection_exists:
                cursor = await db.execute(
                    """
