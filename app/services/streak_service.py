...
from app.stickers.poses import choose_pose
...
class StreakService:
    def __init__(self, repository: Repository, timezone_name: str):
    ...
    async def register_message(self, message: Message) -> Completion:
        ...
        async with self._lock:
            ...
            record = await self.repository.get_or_create_streak(
                connection_id,
                message.chat.id,
                peer_id,
            )
            ...
            if not refreshed:
                return Completion(False)
            if refreshed.owner_sent_day != today or refreshed.peer_sent_day != today:
                return Completion(False)

            # Pose is selected before the atomic completion; on retries only one completion wins.
            projected_days = record.current_streak + 1 if record.last_completed_day == yesterday else 1
            pose = choose_pose(projected_days, record.last_pose)
            ...
            return Completion(completed, days, pose if completed else None)
