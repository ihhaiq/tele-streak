...
class ChannelStreakService:
    ...
    async def register_post(
        self,
        message: Message,
    ) -> tuple[ChannelStreak | None, bool, bool]:
        ...
        async with lock:
            ...
            if (
                ...
                and streak.last_completed_day is not None
                and streak.last_completed_day < yesterday
            ):
                broken = await self.repository.process_missed_day(
                    message.chat.id,
                    today=day,
                    missed_day=yesterday,
                )

            updated, completed = await self.repository.record_post(
                message.chat.id, day, author,
                getattr(getattr(message, "from_user", None), "id", None),
            )
            return updated, completed, broken
